...
int scroll() {
    char* vga = VIDEO_ADDRESS;
    int one_row = (MAX_COLS * 2);
    char* start = VIDEO_ADDRESS + one_row;
    int len = MAX_CELLS - one_row;
    memory_copy(start, vga, len);

    int cur_cell = MAX_CELLS - one_row;

    while (cur_cell <= MAX_CELLS) {
        print_char(0, cur_cell);
        ++cur_cell;
    }

    cur_cell = MAX_CELLS - one_row;

    set_cursor_position(cur_cell);
    return cur_cell;
}
